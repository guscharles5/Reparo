// Fichier : partnerWebhook.js
// Rôle : construit le payload adapté au CRM du partenaire et envoie le webhook de fin de diagnostic (avec signature HMAC)
// Dépendances : node:crypto, table Supabase partners (webhook_url, webhook_secret, crm_type, actif)
...
// Ce que fait cette fonction : envoie le webhook de fin de diagnostic à l'URL du partenaire, signé en HMAC si un secret est configuré, avec timeout optionnel
// Paramètres attendus : partnerRow (ligne partners avec webhook_url/webhook_secret/actif), payload (déjà transformé selon le crm_type), { timeoutMs }
// Ce qu'elle retourne : { skipped: true } si webhook inactif, sinon { ok, status } ou { ok: false, error/timeout }
