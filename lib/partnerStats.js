// Fichier : partnerStats.js
// Rôle : transforme les lignes "conversations" en statuts et KPIs (résolution, NPS avec N par segment, économies estimées, bienvenue, escalades) pour l'espace partenaire
// Dépendances : table Supabase conversations (consommée en entrée, pas requêtée ici)
...
// Ce que fait cette fonction : calcule l'ensemble des KPIs partenaire (taux de résolution, NPS, économies, mode bienvenue, rétention, escalades) à partir des lignes de conversations
// Paramètres attendus : rows (lignes conversations), coutInterventionEvitee (coût unitaire d'une intervention évitée), ouverturesLien (nb d'ouvertures du lien bienvenue loguées)
// Ce qu'elle retourne : un objet KPIs structuré (diagnostics, NPS, économies, bienvenue, escalades, npsParParcours)
export const buildKpis = (rows, coutInterventionEvitee, ouverturesLien = 0) => {
  const now = new Date()
  ...
  // Taux de rétention Mode Bienvenue : % d'utilisateurs avec une conv
  // Bienvenue qui ont une 2e conversation (tout mode confondu) dans les 30
  // jours suivant leur 1re conversation Bienvenue.
  const rowsByUser = {}
  rows.forEach(r => {
    if (!r.user_id) return
  ...
  const retainedUsers = [...bienvenueUsers].filter(uid => {
    const userRows = rowsByUser[uid] || []
    const firstBienvenue = userRows.filter(r => r.mode === 'bienvenue').sort((a, b) => new Date(a.created_at) - new Date(b.created_at))[0]
    if (!firstBienvenue) return false
    const firstTime = new Date(firstBienvenue.created_at).getTime()
    return userRows.some(r => r !== firstBienvenue && new Date(r.created_at).getTime() > firstTime && new Date(r.created_at).getTime() <= firstTime + THIRTY_DAYS_MS)
  }).length
  const tauxRetentionBienvenue = bienvenueUsers.size > 0
    ? Math.round((retainedUsers / bienvenueUsers.size) * 100)
    : null

  // Taux d'ouverture du lien Bienvenue : ouvertures réellement loguées
  // (table bienvenue_ouvertures, une ligne par visite) vs activations
  // effectives de l'écran Bienvenue (conversations en mode bienvenue).
  // Plafonné à 100% : des conversations Bienvenue peuvent exister sans
  // ouverture de lien loguée (deep link direct, historique antérieur au
  // tracking), ce qui sinon produirait un taux > 100% absurde.
  const tauxOuvertureLien = ouverturesLien > 0
    ? Math.min(100, Math.round((bienvenueRows.length / ouverturesLien) * 100))
    : null
  ...
  // NPS segmenté par parcours utilisateur — retourne { value, n } plutôt
  // qu'un simple nombre : un parcours avec 2 réponses peut afficher une
  // moyenne qui a l'air précise mais qui ne veut statistiquement rien dire.
  // L'UI doit afficher n à côté de value pour ne pas induire en erreur.
  const npsParcoursAvg = (parcours) => {
    const seg = npsRows.filter(r => r.nps_parcours === parcours)
    return {
      value: seg.length > 0 ? Math.round((seg.reduce((s, r) => s + r.nps_score, 0) / seg.length) * 10) / 10 : null,
      n: seg.length,
    }
  }
  ...
  return {
    ...
    bienvenue: {
      ...
      tauxRetention: tauxRetentionBienvenue,
      ouverturesLien,
      tauxOuverture: tauxOuvertureLien,
    },
    diagnostic: {
