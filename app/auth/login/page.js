...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (error) { console.error('OAuth error:', error.message); setLoading(false) }
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: PRIMARY, borderRadius: '20px', width: '80px', height: '80px',
      ...
      <p style={{ fontSize: '15px', color: '#888', textAlign: 'center', marginBottom: '40px', lineHeight: '1.5', maxWidth: '280px' }}>
        Votre expert en dépannage électroménager
      </p>
      <button onClick={handleGoogle} disabled={loading} style={{
        width: '100%', maxWidth: '360px', background: 'white', border: '1.5px solid #ddd',
        borderRadius: '14px', padding: '16px', fontWeight: '700', fontSize: '16px',
        cursor: loading ? 'wait' : 'pointer', display: 'flex', alignItems: 'center',
        justifyContent: 'center', gap: '12px', fontFamily: 'Nunito, sans-serif',
        boxShadow: '0 2px 12px rgba(0,0,0,.08)', marginBottom: '12px', color: '#333',
