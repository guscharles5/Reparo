...
// Supabase chargé dynamiquement côté client
let supabase = null;
  ...
  const AuthScreen = () => (
    <div className="slide-in" style={{ minHeight: "100vh", background: "white", display: "flex", flexDirection: "column", padding: "48px 24px 32px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "20px" }}>
        <button onClick={async () => { if (supabase) await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: window.location.origin + "/auth/callback" } }); }}
